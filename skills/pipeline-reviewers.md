...
- **Story terms.json form completeness (stories only).** For every entry in `terms.json` whose `id` starts with `v_` and whose surface looks like a conjugated form (surface ends in た、て、ない、ます、ません、ました、たい、よう、ながら、べき, or any hiragana that is not the dictionary/base-form surface of the glossary entry): verify `form` is NOT null. `form: null` is permitted only for: plain nouns, na-adjective base forms, adverbs, particles, question words, the purpose-construction masu-stem+に (e.g. 買いに), and genuinely unresolvable compound surfaces (where no single `conjugation_rules.json` key covers the surface — these must be rare and commented). Masu-stem surfaces used as compound-verb prefixes (e.g. `行き` in `行き始めた`, `話し` in `話し始めた`) must use `"form": "masu_stem"` — NOT `form: null`. Any conjugated verb surface with `form: null` that does not fall into one of the permitted categories above is a **hard fail**: return to Agent 2 with the surface and the correct form string to use. If no valid form string exists in `conjugation_rules.json`, flag to Agent 1 to add the entry — never accept `form: null` as a substitute for a missing form string.
- **Story terms.json — past potential negative form (stories only).** For any surface ending in 〜えなかった or 〜られなかった (e.g. `読めなかった`, `食べられなかった`), verify `form` is `plain_potential_past_negative` — NOT `plain_potential_negative`. The non-past form produces 読めない; applying it to a past surface shows the wrong form in the chip modal. **Hard fail** if wrong form is used.
- **Story terms.json — compound verb root ID (stories only).** For any compound key that ends in なる/なくなる/くなる (e.g. `"分からなくなる"`, `"少なくなる"`), verify the `id` points to the SEMANTICALLY ROOT verb (the verb being negated or changed), NOT v_naru. Tapping the chip should teach the root concept. Only standalone chips for the なる verb itself (no preceding concept) should use v_naru. **Hard fail** if id is v_naru on a compound that encodes a different root verb.
- **Story terms.json — no te-form + いる compound keys (stories only).** Scan all terms.json keys for any surface matching the pattern `[verb-te] い~` (e.g. `"知って います"`, `"分かって いない"`). These must NOT exist as compound keys — they must always be split into two chips. Verify standalone entries exist for every いる-family form used in the story: `"います"`, `"いた"`, `"いない"` (plain_negative), `"いません"` (polite_negative), `"いる"` (plain_form). **Hard fail** if any te+いる compound key is found.
...
| GP-11 | A "how to spot" or identification test that requires already knowing the answer (circular heuristic) | Any identification heuristic must be usable by a learner who does not yet know the answer. The test must be based on properties observable *before* knowing the answer: orthographic cues (e.g. all-kanji compound → na-adj), morphological patterns (e.g. vowel before る → verb class), or semantic categories (e.g. animate/inanimate → いる/ある). Circular tests — "if you need な before a noun" (learner doesn't know whether to use な), "if it uses で for the past" (learner doesn't know which form to use) — are a **hard fail**. When no reliable test exists, state it explicitly: "this is a short fixed list; memorize it." |
...
- Assess: **Native-speaker word usage** — for every flagged vocabulary item, verify it is used in a context where a native speaker would naturally use that word. This is the most important check Agent 4 performs on stories. Grammar-correctness is not sufficient: a sentence can be grammatically valid but semantically wrong if the word's collocational range does not cover the context. Known failure patterns:
  ...
  - **Adjective+場所/物 where the adverb/noun form is idiomatic.** When an い-adjective modifies 場所, 物, or similar generic nouns to express proximity or degree, native Japanese almost always uses the adverb/noun form instead. E.g. `工場の近い場所に` is grammatically valid but unnatural — native speakers say `工場の近くに` (using 近く as a noun). Similarly `広い場所` can often be replaced by `広いところ` or `広くに`. Flag any adjective+場所/物 construction where the adverb form of the adjective would be more idiomatic. **Hard fail.**
  - **Japanese citation structure.** When a character cites or recalls a written quotation in prose, the standard Japanese structure requires `「quoted text」と、source` — quotation marks around the cited phrase and a quotative `と` before the source attribution. A citation written as `quoted text、source` (comma-joined, no 「」 marks, no と) is unnatural and misleading — it reads as two independent clauses rather than a quotation and its source. **Hard fail** — rewrite to `「quoted text」と、source`.
  - **Redundant adverb with thoroughness-implying verb.** Some verbs already contain the idea of repetition or thoroughness in their meaning: `重ねる` (to pile up / repeat), `続ける` (to continue), `繰り返す` (to repeat). Adding `よく` before these is redundant — `よく考えを重ねた` double-encodes thoroughness. Flag any `よく` (or `しっかり`, `きちんと`) immediately before a verb whose core meaning already implies careful repetition. Soft fail — suggest removing the adverb.
...
4. **Identification heuristic soundness** (grammar lessons only) — For any content that gives a "how to spot," "memory trick," or "quick test" rule: verify the test is usable by a learner who does not yet know the answer. A useful test must be based on properties observable *before* knowing the answer: orthographic cues (e.g. all-kanji compound → な-adj), morphological patterns (e.g. vowel before る → verb class), or semantic categories (e.g. animate/inanimate → いる/ある). A test that uses the answer to find the answer is circular — flag it as GP-11. Also check: if no reliable test exists, is that stated honestly? "You basically need to memorize this list" is correct and useful; a fabricated trick that doesn't work is worse than saying nothing. This check applies to all `explanation`, `notes`, and `points` fields in `grammarRule` and `grammarComparison` sections.
