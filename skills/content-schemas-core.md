...
| `conjugationDrill` | `type`, `title`, `instructions`, **`manualProgression: true`** (required), `items[]` | Each item: `verb`, `type`, `reading`, `targetForm`, `answer`, `answerReading`, `hint`, `choices[]`. **Always set `manualProgression: true`** — without it the drill auto-advances after 1.4s, which is too fast for students to read the hint. |
| `patternMatch` | `type`, `title`, `instructions`, `items[]` | Each item: `sentence`, `answer` (bool), `explanation`. |
| `sentenceTransform` | `type`, `title`, `instructions`, **`manualProgression: true`** (required), `items[]` | Each item: `given`, `givenReading`, `targetLabel`, `answer`, `answerReading`, `hint`, **`choices[]`** (required — 4 strings; the renderer crashes and leaves the screen blank if `choices` is missing). **Always set `manualProgression: true`** — without it the drill auto-advances after 1.5s. |
