...
[ ] **Story terms.json only — masu-stem compound verb prefixes:** When a masu-stem surface appears as the first element of a compound verb (e.g. `行き` in `行き始めた`, `話し` in `話し始めた`, `考え` in `考え始めた`), use `"form": "masu_stem"` — NOT `form: null`. The purpose-construction exception does NOT apply here because the verb is followed by another verb, not by に.
[ ] **Story terms.json only — narrative scaffold word audit:** Every content word in the story prose (not just the flagged target vocabulary) must have a verified glossary entry. Before writing story.md, grep-check EVERY non-trivial narrative word — verbs, nouns, adjectives — against glossary.N5.json and glossary.N4.json entries. Words that seem basic but are often NOT in the N5/N4 glossary include: かける (掛ける, "to spend/take time"), 掃く, 掃除, 廊下, 机, 椅子, 状態, 自信, 細かい. If a word has no glossary entry, find an alternative that does before writing the sentence — never use a word speculatively and fix it later.
[ ] **Story terms.json only — Japanese citation structure:** When a character recalls or cites a written quotation, the sentence must use 「」 quotation marks around the cited phrase and a quotative `と` before the source: `「引用文」と、ある文書に書いてあった`. The pattern `引用文、ある文書に書いてあった` (comma-joined, no marks, no と) is unnatural and must not be used.
[ ] **Story terms.json only — no greedy adj-adverbial compounds:** Do NOT create compound keys that merge an adjective adverbial suffix into the following verb (e.g. `"なくなります"`, `"よくなくなります"`, `"くなります"`). These cause the longest-match tokenizer to absorb the entire sequence as one chip, swallowing the adj-adverbial chip. Always tag each morpheme separately: `"少なく"` + `"なります"`, `"よく"` + `"なく"` + `"なります"`. Exception: true compound verbs where the entire sequence is a single lexical unit (e.g. `"分からなくなる"` meaning "become unable to understand") MAY use a compound key.
[ ] **Story terms.json only — no compound key whose prefix is a registered verb form:** Do NOT create compound keys whose prefix is itself a separately-registered verb surface (e.g. `"見ながらでも"` where `"見ながら"` is already in terms.json). The app tokenizer fails to match such keys in practice even when the simulation succeeds. Instead, register the verb form and the following particle as two separate chips: `"見ながら"` (v_miru/nagara_form) + `"でも"` (p_demo_but). If the particle meaning needs disambiguation, use a prefix that does NOT start with a registered verb form (e.g. `"図書館でも"` is safe because `"図書館"` is a noun entry, not a conjugated verb form).
[ ] **Story terms.json only — te-form + いる always splits into two chips:** NEVER create compound keys that fuse a て-form verb with a following いる/います/いた/いない/いません (e.g. `"知って います"`, `"分かって いない"`). Always tag them as two separate chips. Ensure standalone entries exist for every いる variant used: `"います"`, `"いた"`, `"いない"` (plain_negative), `"いません"` (polite_negative), `"いる"` (plain_form). Exception: `"書いて あった"` and similar past-result constructions using あった are compound keys because あった is the past of ある, which acts as a different semantic auxiliary.
[ ] **Story terms.json only — past potential negative form string:** Use `plain_potential_past_negative` (not `plain_potential_negative`) for surfaces ending in 〜えなかった / 〜られなかった. `plain_potential_negative` produces 読めない (present), so applying it to 読めなかった (past) shows the wrong form in the chip modal.
[ ] **Story terms.json only — compound verb root ID:** When a compound key represents [verb]-なくなる or [verb]-なる (e.g. `"分からなくなる"`, `"少なくなる"`), set the `id` to the SEMANTICALLY ROOT verb (the one being negated/changed), not v_naru. Tapping the chip should open the concept the student is learning about: `"分からなくなる"` → v_wakaru, `"少なくなる"` → v_sukunai. Only standalone `"なる"` or `"なります"` chips (with no preceding verb concept) should use v_naru.
[ ] **Story terms.json only — negative adverbial of adjective:** When an adjective's negative adverbial form appears as a unit (よくなく = not-good-ly, from よい; 少なくなく = not-few-ly, from 少ない), register it as a compound key pointing to the SOURCE adjective with `form: null`. E.g. `"よくなく": { "id": "v_ii", "form": null }`. Do NOT split it into `"よく"` + `"なく"` — that splits one conceptual unit across two adjective chips.
