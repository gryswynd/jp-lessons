...
window.PracticeModule = {
  start: function(container, sharedConfig, exitCallback) {
    ...
    // --- SETUP ---
    window.KanjiApp = {};
    window.KanjiApp.exit = exitCallback; // Allow internal access
    const REPO_CONFIG = sharedConfig;

    // Inject Fonts (Check if exists first)
    ...
    // Inject CSS (Check if exists)
    if (!document.getElementById('kanji-app-css')) {
        const css = `
            #kanji-app-root {
            ...
            #kanji-app-root .k-exit-btn {
                background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.4);
                color: white; padding: 5px 12px; border-radius: 8px; cursor: pointer; font-size: 0.8rem;
            }
    ...
    // HTML Structure (Modified to include Exit Button)
    appRoot.innerHTML = `
        ...
        <header>
            <span onclick="KanjiApp.showMenu()" style="cursor:pointer">Kanji Master 先生</span>
            <button class="k-exit-btn" onclick="KanjiApp.exit()">Exit</button>
        </header>
        ...
        <div id="k-app-container">
            ...
            <div id="k-view-flash" class="k-hidden" style="width:100%">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px; margin-top:15px; width:100%">
                    ...
                    <button class="k-btn k-btn-sec" id="k-btn-flag" onclick="KanjiApp.flag(this)" style="color:#f39c12; border-color:#f39c12;">Flag 🚩</button>
    ...
    KanjiApp.showMenu = function() {
        kUpdateStats();
        ['k-view-menu','k-view-flash','k-view-quiz'].forEach(i => { const el = document.getElementById(i); if(el) el.classList.add('k-hidden'); });
        const menu = document.getElementById('k-view-menu'); if(menu) menu.classList.remove('k-hidden');
    };
    ...
    KanjiApp.start = function(type, mode, subMode='normal') {
            ...
            activeK.forEach(k => {
                ...
                cs.forEach((c) => {
                    ...
                    if(vObj && !tempMap.has(c)) {
                        tempMap.set(c, { word: c, reading: vObj.reading, meaning: vObj.meaning, lesson: k.lesson, gtype: vObj.gtype, notes: vObj.notes });
                    }
                });
            });
        ...
        ['k-view-menu','k-view-flash','k-view-quiz'].forEach(i => { const el = document.getElementById(i); if(el) el.classList.add('k-hidden'); });
        if(mode === 'flash' || mode === 'flag-review') { kRenderFC(); document.getElementById('k-view-flash').classList.remove('k-hidden'); }
        else { KanjiApp.nextQ(); document.getElementById('k-view-quiz').classList.remove('k-hidden'); }
    };

    KanjiApp.flipCard = function() { const c = document.getElementById('k-fc-card-obj'); if(c) c.classList.toggle('is-flipped'); };
    KanjiApp.move = function(n) { curIdx = (curIdx+n+curSet.length)%curSet.length; kRenderFC(); };
    ...
    KanjiApp.flag = function(btn) {
        ...
        btn.innerText = "Re-queued! ↺"; btn.style.backgroundColor = "#fff3cd";
    ...
    KanjiApp.clearFlag = function(btn) {
        ...
        setTimeout(() => {
            if(curSet.length > 1) { curSet.splice(curIdx, 1); if(curIdx >= curSet.length) curIdx = 0; kRenderFC(); }
            else { alert("All active flags cleared! Returning to menu."); KanjiApp.showMenu(); }
        }, 600);
    };

    KanjiApp.toggleAccordion = function(h) { h.classList.toggle('open'); h.nextElementSibling.classList.toggle('open'); };
    KanjiApp.toggleAll = function(cls, p) { document.querySelectorAll(`.k-chk-${cls}`).forEach(b => { b.checked = p.checked; if(p.checked) activeLessons.add(b.value); else activeLessons.delete(b.value); }); kUpdateStats(); };
    KanjiApp.updateLesson = function(id, cls) { if(activeLessons.has(id)) activeLessons.delete(id); else activeLessons.add(id); const all = document.querySelectorAll(`.k-chk-${cls}`); const checked = document.querySelectorAll(`.k-chk-${cls}:checked`); const p = all[0].closest('.k-lvl-group').querySelector('.k-lvl-header input'); p.checked = (all.length === checked.length); p.indeterminate = (checked.length > 0 && checked.length < all.length); kUpdateStats(); };
    ...
    KanjiApp.nextQ = function() {
        ...
        if(curMode==='quiz-meaning') {
            ...
            if(effectiveSubMode === 'normal') { q='Select readings'; m=curQItem.kanji; a=[curQItem.on,curQItem.kun].filter(x=>x).join(' / '); let safe=0; while(dists.length<3 && safe++<50) { let r=curSet[Math.floor(Math.random()*curSet.length)]; let x=[r.on,r.kun].filter(y=>y).join(' / '); if(x!==a && !dists.includes(x)) dists.push(x); } }
            else { q='Which Kanji reads...'; m=[curQItem.on,curQItem.kun].filter(x=>x).join(' / '); a=curQItem.kanji; dists = kRand(curSet, 'kanji', a); }
        } else if(curMode==='quiz-vocab') {
            q = 'What is the meaning?'; m = curQItem.word; a = curQItem.meaning; let safe=0; while(dists.length<3 && safe++<50) { let r=curSet[Math.floor(Math.random()*curSet.length)]; if(r.meaning && r.meaning!==a && !dists.includes(r.meaning)) dists.push(r.meaning); }
            ...
            q=`What is the ${lbls[f]} form?`; m=`${curQItem.meaning} (${curQItem.dict})`; a=curQItem[f]; let safe=0; while(dists.length<3 && safe++<50) { let r=curSet[Math.floor(Math.random()*curSet.length)]; if(r[f]!==a && !dists.includes(r[f])) dists.push(r[f]); }
        }
        curAns = a;
        setTxt('k-q-ask', q); setTxt('k-q-main', m);
        ...
        const mainEl = document.getElementById('k-q-main'); if(mainEl) mainEl.style.fontSize = isBig ? '5rem' : '2.5rem';
        kRenderOpts(a, dists);
    };

    KanjiApp.check = function(sel, btn) {
        const nextBtn = document.getElementById('k-q-next'); if(nextBtn && !nextBtn.classList.contains('k-hidden')) return;
        ...
        if(sel===curAns) {
            ...
            if(readEl) { if(curMode === 'quiz-meaning') { readEl.innerText = [curQItem.on, curQItem.kun].filter(x => x).join(' / '); readEl.classList.remove('k-hidden'); } else if (curMode === 'quiz-reading') { readEl.innerText = curQItem.meaning; readEl.classList.remove('k-hidden'); } }
            if(curMode === 'quiz-vocab' && quizPhase === 1) {
                if(msg) { msg.innerText = "Correct! BONUS: Select the Reading"; msg.style.color="#155724"; msg.style.background="#d4edda"; msg.classList.remove('k-hidden'); }
                setTimeout(() => { quizPhase = 2; setTxt('k-q-ask', "What is the reading?"); let dists = []; let safe=0; while(dists.length<3 && safe++<50) { let r=curSet[Math.floor(Math.random()*curSet.length)]; if(r.reading && r.reading!==curQItem.reading && !dists.includes(r.reading)) dists.push(r.reading); } curAns = curQItem.reading; if(msg) msg.classList.add('k-hidden'); kRenderOpts(curAns, dists); }, 800); return;
            }
        ...
        setTxt('k-streak', curStreak); setTxt('k-best', curBest);
        if(msg) msg.classList.remove('k-hidden'); if(nextBtn) nextBtn.classList.remove('k-hidden');
    };

    function kRand(set, key, exc) { let res=[]; let s=0; while(res.length<3 && s++<100) { let r=set[Math.floor(Math.random()*set.length)][key]; if(r!==exc && !res.includes(r)) res.push(r); } return res; }
    function kRenderOpts(ans, dists) { let opts = [ans, ...dists].sort(()=>Math.random()-0.5); const c = document.getElementById('k-q-opts'); if(!c) return; c.innerHTML = ''; opts.forEach(o => { let b = document.createElement('div'); b.className='k-opt'; b.innerText=o; b.onclick = ()=>KanjiApp.check(o, b); c.appendChild(b); }); }

    function kRenderFC() {
        const d = curSet[curIdx];
        setTxt('k-fc-progress', `Card ${curIdx+1} / ${curSet.length}`);
        const cardObj = document.getElementById('k-fc-card-obj'); if(cardObj) cardObj.classList.remove('is-flipped');
        const stamp = document.getElementById('k-fc-flagged-icon');
        if(stamp) { const kKey = d.kanji || d.word || d.dict; if((d.isRequeued || activeFlags[kKey]) && curMode !== 'flag-review') stamp.classList.remove('k-hidden'); else stamp.classList.add('k-hidden'); }
        const actBtn = document.getElementById('k-btn-flag');
        if(actBtn) { if(curMode === 'flag-review') { actBtn.innerText = "Clear Flag 🏳️"; actBtn.onclick = function() { KanjiApp.clearFlag(this); }; actBtn.style.color = "#2ed573"; actBtn.style.borderColor = "#2ed573"; actBtn.style.backgroundColor = ""; } else { actBtn.innerText = "Flag 🚩"; actBtn.onclick = function() { KanjiApp.flag(this); }; actBtn.style.color = "#f39c12"; actBtn.style.borderColor = "#f39c12"; actBtn.style.backgroundColor = ""; } }
        const backEl = document.getElementById('k-fc-back-content');
        let renderType = curType; if(curType === 'mixed' && d._type) renderType = d._type;

        if(renderType==='kanji') {
            setTxt('k-fc-lbl', curMode === 'flag-review' ? `🚩 Flags: ${flagCounts[d.kanji]||0}` : d.lesson);
            setTxt('k-fc-main', d.kanji); setTxt('k-fc-sub', "");
            ...
            const flags = flagCounts[d.kanji] || 0; if(flags > 0) h += `<div style="text-align:center; color:#ff4757; font-weight:700; margin-bottom:15px; font-size:0.8rem;">🚩 Flagged: ${flags} times</div>`;
            h += `<div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:15px; width:100%"><div style="background:#f8f9fa; padding:10px; border-radius:8px; text-align:center"><div style="font-size:0.7rem; color:#aaa; font-weight:700;">ON-YOMI</div><div style="font-size:1.2rem; font-weight:600;">${d.on||'-'}</div></div><div style="background:#f8f9fa; padding:10px; border-radius:8px; text-align:center"><div style="font-size:0.7rem; color:#aaa; font-weight:700;">KUN-YOMI</div><div style="font-size:1.2rem; font-weight:600;">${d.kun||'-'}</div></div></div>`;
            h += `<div class="k-lbl" style="text-align:left;margin-top:10px;border-bottom:1px solid #eee;">Compounds</div><table class="k-tbl">`;
            const cs=(d.compounds||'').split(';'), rs=(d.comp_readings||'').split(';'), ms=(d.comp_meanings||'').split(';'); cs.forEach((c,i)=>{ if(c) h+=`<tr><td style="font-weight:bold; font-size:1.2rem">${c}</td><td><div style="font-size:1rem">${rs[i]||''}</div><div style="color:#747d8c; font-size:0.9rem">${ms[i]||''}</div></td></tr>`; });
            if(backEl) backEl.innerHTML = h+'</table>';
        } else if(renderType==='vocab') {
            setTxt('k-fc-lbl', curMode === 'flag-review' ? `🚩 Flags: ${flagCounts[d.word]||0}` : "Vocabulary");
            setTxt('k-fc-main', d.word); setTxt('k-fc-sub', "");
            ...
            const flags = flagCounts[d.word] || 0; if(flags > 0) h += `<div style="text-align:center; color:#ff4757; font-weight:700; margin-top:15px; font-size:0.8rem;">🚩 Flagged: ${flags} times</div>`;
            if(backEl) backEl.innerHTML = h;
        } else {
            setTxt('k-fc-lbl', curMode === 'flag-review' ? `🚩 Flags: ${flagCounts[d.kanji||d.dict]||0}` : 'Verb');
            setTxt('k-fc-main', d.kanji==='-'?d.dict:d.kanji); setTxt('k-fc-sub', "");
    ...
    function kUpdateStats() {
        ...
        activeK.forEach(k => { const cs = (k.compounds||'').split(';'); cs.forEach(c => { if(c) uniqueVocab.add(c); }); });
    ...
    (async function() {
        try {
            ...
            allVocab.forEach(v => { DB.vocabMap.set(v.surface, v); });

            DB.kanji = raw.filter(i => i.type === 'kanji').map(k => {
                const compounds=[], comp_readings=[], comp_meanings=[];
                allVocab.forEach(v => { if (v.surface.includes(k.surface) && compounds.length < 5) { compounds.push(v.surface); comp_readings.push(v.reading); comp_meanings.push(v.meaning); } });
                return { class: k.lesson||"General", lesson: k.lesson||"Other", kanji: k.surface, on: k.on||"", kun: k.kun||"", meaning: k.meaning, compounds: compounds.join(';'), comp_readings: comp_readings.join(';'), comp_meanings: comp_meanings.join(';') };
            });
            ...
            DB.verb = baseVerbs.map(base => {
                ...
                return { kanji: base.surface, dict: base.surface, reading: base.reading, meaning: base.meaning, masu: getForm('__polite'), te: getForm('__te'), nai: getForm('__negative'), ta: getForm('__past'), potential: getForm('__potential') };
            });

            const lessonMap = {};
            DB.kanji.forEach(k => { if(!k.lesson) return; if(!lessonMap[k.lesson]) lessonMap[k.lesson] = { id: k.lesson, topic: `Lesson ${k.lesson}`, kanji: [] }; lessonMap[k.lesson].kanji.push(k.kanji); });
            DB.lessons = Object.values(lessonMap).map(l => ({ id: l.id, topic: l.topic, kanji_list: l.kanji.join(', ') }));
            DB.lessons.sort((a,b) => { const pa = a.id.replace('N','').split('.').map(Number); const pb = b.id.replace('N','').split('.').map(Number); if(pa[0]!==pb[0]) return pa[0]-pb[0]; return (pa[1]||0) - (pb[1]||0); });

            const container = document.getElementById('k-lesson-container');
            if(container) {
                container.innerHTML = ''; const groups = {};
                DB.lessons.forEach(l => { const cls = l.id.split('.')[0] || "Other"; if(!groups[cls]) groups[cls] = []; groups[cls].push(l); activeLessons.add(l.id); });
                Object.keys(groups).sort().forEach(cls => {
                    const div = document.createElement('div'); div.className = 'k-lvl-group';
                    div.innerHTML = `<div class="k-lvl-header" onclick="KanjiApp.toggleAccordion(this)"><input type="checkbox" class="k-chk" checked onclick="event.stopPropagation(); KanjiApp.toggleAll('${cls}', this)"><div class="k-lvl-title">${cls}</div><div class="k-lvl-arrow">▼</div></div><div class="k-lvl-list">${groups[cls].map(l => `<div class="k-lesson-row"><input type="checkbox" class="k-chk k-chk-${cls}" value="${l.id}" checked onchange="KanjiApp.updateLesson('${l.id}', '${cls}')"><div class="k-l-info" onclick="this.previousElementSibling.click()"><div class="k-l-topic">${l.topic}</div><div class="k-l-kanji">${l.kanji_list}</div></div></div>`).join('')}</div>`;
                    container.appendChild(div);
                });
            }
            kUpdateStats();
            const loader = document.getElementById('k-loader'); if(loader) loader.classList.add('k-hidden');
        } catch(e) { console.error(e); const errBox = document.getElementById('k-error-box'); if(errBox) { errBox.innerText = "Error: " + (e.message || "Unknown error"); errBox.classList.remove('k-hidden'); } }
    })();
