...
10. **Grammar lesson teaches a new conjugation form with no `conjugation_rules.json` entry** — When a grammar lesson formally introduces a new verb or adjective form (e.g. G12 introduces `potential`, G21 introduces `passive`, G18 introduces `tari_form` and `nagara_form`), that form must exist as a top-level key in `conjugation_rules.json` with complete rules before Agent 2 can use it. If the entry is missing:
