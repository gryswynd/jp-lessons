...
| 後で | あとで | later; afterwards | `v_atode` | Fixed adverbial expression — tag as a single unit, no separate p_de |
| 後 (standalone) | あと | after / later | `v_ato` | N4+ constructions: 食べた後、三時間後 etc. |

**Unlike 何/なに/なん, the written form alone disambiguates — no phonological context rules needed:**

- Token is `後ろ` → `v_ushiro`
- Token is `後で` → `v_atode` (single tag — do NOT split into v_ato + p_de; the frontend matches on whole space-delimited tokens, so the surface must equal the full token)
- Token is `後` standalone (no ろ, no で) → `v_ato` (N4+ level)

**Why `v_atode` and not `v_ato` + `p_de`:** The frontend matches vocab surface strings against space-delimited tokens in `jp` text. `後で` is a single token; surface `後` does not match it. `v_atode` has surface `後で`, so it matches correctly and is tappable.
...
36. **後 tagged as v_ushiro regardless of meaning** — 後 has two completely distinct words: **後ろ** (`v_ushiro`, うしろ, spatial "behind") and **後** (`v_ato`, あと, temporal "after/later"). Unlike 何, the written form alone disambiguates them — no context rules needed. **Rule:** Token `後ろ` (with ろ) → `v_ushiro`. Token `後で` (later/afterwards) → `v_atode` as a single unit — do NOT split into `v_ato` + `p_de`, because the frontend matches surfaces against whole space-delimited tokens. Token `後` standalone (no ろ, no で) → `v_ato` (N4+ constructions like 食べた後). Using `v_ushiro` for temporal 後 is semantically wrong (it displays the wrong reading and meaning to the student). Never use `k_ushiro` in conversation, reading, or drill `terms` — it is only for the kanjiGrid. Compounds using the on-reading (午後, 後半, etc.) have their own dedicated IDs.
